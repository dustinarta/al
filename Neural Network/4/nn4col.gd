...
func add_output(count:int = 1):
	var new_weight:PackedFloat64Array
	new_weight.resize(count * input_size)
	for w in range(count * input_size):
...
func forward(inputs:PackedFloat64Array):
	...
	for o in range(output_size):
		...
		for i in range(input_size):
			res += inputs[i] * weights[o * input_size + i]
		results[o] = res
	results = activations(results, activation)
	forward_result = results
	return results

func train(inputs:PackedFloat64Array, expected:PackedFloat64Array, rate:float = 0.01):
	var results
	var derivatives
	...
	for i in range(input_size):
		for o in range(output_size):
			weights[i + output_size * o] += errors[o] * derivatives[o] * inputs[i] * rate
...
func train_with_error(inputs:PackedFloat64Array, errors:PackedFloat64Array):
	...
	for i in range(input_size):
		var error:float = 0.0
		for o in range(output_size):
			error += errors[o] * derivatives[o] * weights[o + input_size * i]
		futureerrors[i] = error
	
	for i in range(input_size):
		for o in range(output_size):
			weights[o + input_size * i] += errors[o] * derivatives[o] * inputs[i]
